...
**LoFT** (Long-tailed semi-supervised learning via parameter-efficient Fine-Tuning) is a specialized framework designed to address the critical challenges of [[long-tailed-semi-supervised-learning-ltssl|Long-tailed semi-supervised learning (LTSSL)]]. LTSSL presents a dual difficulty: the severe scarcity of samples in "tail" classes and the presence of noise within [[pseudo-labels|Pseudo-labels]] generated during the learning process.

### The Problem with Training from Scratch
Most traditional LTSSL methodologies focus on training models from scratch. The authors argue that this approach is fundamentally flawed for long-tailed distributions, as it frequently leads to high [[mitigating-structural-overfitting-a-distribution-aware-rectification-framework-f|Overfitting]] rates, excessive model overconfidence, and the propagation of low-quality, inaccurate pseudo-labels.

### The LoFT Approach
LoFT proposes a paradigm shift by moving away from scratch-based training toward the fine-tuning of [[a-family-of-open-time-series-foundation-models-for-the-radio-access-network|Foundation Models]]. This transition is supported by two primary theoretical pillars:

1.  **Complexity Reduction**: The researchers theoretically demonstrate that employing a foundation model significantly reduces [[hypothesis-complexity|Hypothesis Complexity]]. This reduction tightens the [[generalization-bound|Generalization Bound]] and effectively minimizes the [[balanced-posterior-error-bpe|Balanced Posterior Error (BPE)]].
...
By utilizing [[loft-parameter-efficient-fine-tuning-for-long-tailed-semi-supervised-learning-in|Parameter-Efficient Fine-Tuning]] (PEFT), LoFT maintains the high-quality feature representations of the base model while adapting to the specific long-tailed distribution of the target task.

### Expansion to Open-World Scenarios
To address real-world complexities, the paper introduces **LoFT-OW** (LoFT under Open-World scenarios). In practical applications, unlabeled datasets often contain [[out-of-distribution-ood|Out-of-Distribution (OOD)]] samples—data that does not belong to any of the predefined classes. LoFT-OW is specifically engineered to improve discriminative ability in these [[open-world-scenarios|Open-World Scenarios]], preventing unknown classes from corrupting the learned representations of the known classes.

Experimental evaluations across various benchmarks indicate that LoFT and LoFT-OW achieve superior performance compared to existing state-of-the-art methods in [[benchmarking-deep-learning-for-future-liver-remnant-segmentation-in-colorectal-l|Deep Learning]].