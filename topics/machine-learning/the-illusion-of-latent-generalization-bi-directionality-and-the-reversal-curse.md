...
* **Geometric Divergence:** The researchers found that the [[indexing geometry]] used to store these facts differs significantly between MLM and decoder-only masking-based training.

## Implications for AI
The paper provides a cautionary note for the development of [[Large Language Models]]. It suggests that improvements in reversal performance may be an "illusion" of intelligence. While architectural changes can "fix" the reversal behavior in benchmark tests, they may not be inducing the high-level, abstract reasoning or [[generalization]] that researchers hope to achieve. Instead, the model may simply be performing more efficient, multi-directional pattern matching.