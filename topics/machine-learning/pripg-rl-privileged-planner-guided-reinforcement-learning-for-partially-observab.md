...
**PriPG-RL** (Privileged Planner-Guided Reinforcement Learning) is a novel framework designed to train [[Reinforcement Learning]] (RL) policies in environments characterized by [[Partial Observability]]. The paper addresses the inherent difficulties of training agents within a [[Partially Observable Markov Decision Process]] (POMDP), where the agent's sensors provide only a lossy or incomplete projection of the true environmental state.

## Core Framework

The fundamental concept behind PriPG-RL is the introduction of a "privileged planner" that is available exclusively during the training phase. Unlike the learning agent, this planner has access to privileged state information and an approximate [[Dynamical Model]] of the system. By leveraging this extra information during training, the framework provides a high-quality guidance signal that is absent during real-world deployment.
...
2.  **Real-World Deployment:** The approach was successfully transferred to physical hardware using a **Unitree Go2** quadruped robot. The robot demonstrated robust navigation capabilities within complex, obstacle-rich environments, proving the framework's efficacy in real-world [[Robotics]]-based [[Autonomous Systems]].