...
2.  **Reward Shaping**: Once the dataset has been "cleaned," the repaired trajectories are used to extract potential functions. These functions serve as a reward-shaping mechanism for [[Reinforcement Learning]], guiding the agent toward task-consistent regions of the state space without needing an explicit kinematic model of the agent.
...
The results indicate that this framework significantly enhances data efficiency, allowing agents to learn high-quality policies from "dirty" datasets that would otherwise be unsuitable for standard [[Machine Learning]] training pipelines. This approach paves the way for more robust autonomous systems in environments where high-fidelity human demonstrations are difficult to obtain.