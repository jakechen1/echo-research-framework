...
The authors propose an alternative viewpoint, suggesting that non-reciprocity can be decomposed into two distinct elements: systematic [[Scale Variation]] and random [[Noise]]. The paper introduces an additive model where the underlying comparison matrix remains consistent, but includes a symmetric component that reflects changes in the evaluation scale.
