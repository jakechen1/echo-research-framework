...
**SignalClaw** is a novel framework designed for [[Traffic Signal Control (TSC)]], aimed at bridging the gap between the high performance of [[Reinforcement Learning]] and the necessity for human-interpretable control logic. The framework utilizes [[Large Language Models (LLMs)]] as evolutionary skill generators to synthesize, refine, and deploy executable, self-documenting control programs.

## Overview

Traditional approaches to traffic management often face a trade-off between efficiency and transparency. While neural-based policies like [[Deep Q-Network (DQN)]] can optimize flow, they operate as "black boxes" that are difficult for engineers to audit. Conversely, traditional [[Program Synthesis]] often relies on overly restrictive domain-specific languages. 
...
*   **Event-Driven Composition:** Utilizing [[TraCI]], the framework employs an event detector to identify specific scenarios, such as emergency vehicle approach, transit priority needs, or unexpected congestion.
