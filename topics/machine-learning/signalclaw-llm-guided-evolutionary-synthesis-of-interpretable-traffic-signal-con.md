...
**SignalClaw** is a novel framework designed for [[traffic-signal-control-tsc|Traffic Signal Control (TSC)]], aimed at bridging the gap between the high performance of [[deepsearch-overcome-the-bottleneck-of-reinforcement-learning-with-verifiable-rew|Reinforcement Learning]] and the necessity for human-interpretable control logic. The framework utilizes [[large-language-models-llms|Large Language Models (LLMs)]] as evolutionary skill generators to synthesize, refine, and deploy executable, self-documenting control programs.

## Overview

Traditional approaches to traffic management often face a trade-off between efficiency and transparency. While neural-based policies like [[deep-q-network-dqn|Deep Q-Network (DQN)]] can optimize flow, they operate as "black boxes" that are difficult for engineers to audit. Conversely, traditional [[scientific-graphics-program-synthesis-via-dual-self-consistency-reinforcement-le|Program Synthesis]] often relies on overly restrictive domain-specific languages. 

SignalClaw addresses this by using LLMs to evolve "skills"—sets of executable code accompanied by natural language rationales and selection guidance. The evolutionary loop is driven by converting [[simulation-metrics|Simulation Metrics]] (such as queue percentiles, delay trends, and stagnation) into natural language feedback. This feedback instructs the LLM on how to improve the control logic in subsequent generations.
...
In evaluations conducted via the [[sumo|SUMO]] simulator, SignalClaw demonstrated superior performance in high-priority scenarios:
...
The framework shows that evolving skills from simple linear rules to complex [[automated-program-synthesis|Automated Program Synthesis]] strategies can maintain stable overall delays even in mixed-event environments.