...
The paper "Polynomial-Time Algorithm for Thiele Voting Rules with Voter Interval Preferences" presents a significant breakthrough in the field of [[Computational Social Choice]]. The research provides a polynomial-time algorithm for computing an optimal committee of size $k$ under any given [[Thiele voting rules]]. This is specifically achieved within the "Voter Interval domain," a setting where voters can be ordered such that each candidate is approved by a consecutive sequence of voters.
