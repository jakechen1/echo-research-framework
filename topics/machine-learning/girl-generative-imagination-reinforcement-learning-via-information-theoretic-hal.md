...
**GIRL** (Generative Imagination Reinforcement Learning) is a novel framework designed to address a fundamental failure mode in [[reinvbc-a-model-based-reinforcement-learning-approach-to-vehicle-braking-control|Model-based Reinforcement Learning]] (MBRL): the accumulation of modeling errors during long-horizon planning. In traditional MBRL, agents optimize policies by simulating trajectories within a learned [[causalvae-as-a-plug-in-for-world-models-towards-reliable-counterfactual-dynamics|World Models]] environment. However, as these "imagined" rollouts extend, small prediction errors compound, causing the latent trajectories to drift away from the real training manifold—a phenomenon often referred to as "hallucination."

### Key Innovations

GIRL introduces two primary mechanisms to stabilize the imagination process:

1.  **Cross-Modal Grounding**: To prevent the model from predicting physically implausible states, GIRL utilizes a grounding signal derived from a frozen [[a-family-of-open-time-series-foundation-models-for-the-radio-access-network|Foundation Models]] architecture, specifically [[dinov2|DINOv2]]. This anchors the latent transition prior to a semantically consistent embedding space, effectively penalizing predictions that deviate from known visual features.
...
Experimental results across several benchmark suites—including [[deepmind-control|DeepMind Control]], Adroit Hand Manipulation, and Meta-World—demonstrate the effectiveness of the approach. Compared to [[dreamerv3|DreamerV3]], GIRL reduces latent rollout drift by between 38% and 61%. Additionally, the framework outperforms [[td-mpc2|TD-MPC2]] in environments characterized by sparse rewards and high-contact dynamics. For deployment efficiency, a distilled-prior variant is also introduced to reduce inference overhead.