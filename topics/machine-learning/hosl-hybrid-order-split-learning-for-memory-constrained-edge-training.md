...
**HOSL** (Hybrid-Order Split Learning) is a novel framework designed to facilitate the collaborative training of [[large-language-models-llms|Large Language Models (LLMs)]] between resource-constrained edge devices and compute-rich servers. The framework operates within the paradigm of [[split-learning-sl|Split Learning (SL)]], where the architecture of a neural network is partitioned across a network boundary to balance computational load.

## The Optimization Trade-off

Traditionally, Split Learning systems have relied on [[first-order-fo-optimization|First-Order (FO) Optimization]]. While effective, FO optimization requires edge clients to store intermediate quantities, such as activations, to facilitate backpropagation. In the context of massive models, this memory overhead often negates the primary benefits of offloading computation to a server.

An alternative approach is [[zeroth-order-zo-optimization|Zeroth-Order (ZO) Optimization]]. Because ZO optimization eliminates the need for backpropagation, it significantly reduces the client's memory footprint. However, this method is notorious for slow convergence rates and degraded model performance, making it difficult to train complex architectures efficiently.
...
Extensive testing on [[opt-models|OPT Models]] (125M and 1.3B parameters) demonstrates the practical advantages of HOSL:
