...
2.  **Neural Scaling Law Fitting**: The framework fits [[neural-scaling-laws|Neural Scaling Laws]] to each specific domain, mapping how data volume from that domain correlates with target evaluation metrics.
...
The researchers applied MOSAIC to an [[end-to-end-learning|End-to-End Learning]] planner for autonomous vehicles. The model was evaluated using the Extended Predictive Driver Model Score (EPDMS), which aggregates various driving rule compliance metrics. 
