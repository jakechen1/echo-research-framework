...
[[accelerated-sinkhorn-algorithms-for-partial-optimal-transport|Partial Optimal Transport]] (POT) is a specialized variant of the classical [[fast-and-interpretable-protein-substructure-alignment-via-optimal-transport|Optimal Transport]] problem. While standard optimal transport requires the total mass of two distributions to be identical, POT allows for the transport of only a fraction of the total mass between two distributions. This capability is essential when working with [[marginal-distributions|marginal distributions]] of unequal sizes or datasets characterized by significant [[outliers]].

Despite the utility of POT, existing [[sinkhorn-based-methods|Sinkhorn-based methods]]—which use entropic regularization to make the problem computationally tractable—suffer from suboptimal complexity bounds in the POT setting. These limitations often hinder the scalability of these algorithms when applied to large-scale [[a-comparative-analysis-of-machine-learning-models-in-shap-analysis|machine learning]] tasks or complex [[computer-vision|computer vision]] datasets.

## The ASPOT Algorithm

The authors propose **ASPOT**, an algorithm that integrates [[alternating-minimization|alternating minimization]] with [[nesterov-style-acceleration|Nesterov-style acceleration]] specifically optimized for the POT framework. The primary contribution is a significant reduction in computational complexity, achieving a bound of $\mathcal{O}(n^{7/3}\varepsilon^{-5/3})$.
...
The efficiency gains provided by ASPOT have significant implications for [[computational-geometry|computational geometry]], [[pattern-recognition|pattern recognition]], and any domain utilizing [[distribution-matching|distribution matching]] where data noise or unbalanced samples are prevalent.