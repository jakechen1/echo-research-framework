...
**NAIMA** (Neural Attention for Implicit Multi-token Alignment) is an advanced framework designed to improve the accuracy of [[Guided Depth Super-Resolution]] (GDSR). The primary goal of GDSR is to enhance the resolution of a low-resolution depth map by utilizing the high-resolution structural information found in a corresponding RGB image.
