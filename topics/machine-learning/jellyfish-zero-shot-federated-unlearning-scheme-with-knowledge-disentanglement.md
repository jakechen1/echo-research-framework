...
**Jellyfish** is an innovative [[Zero-Shot Federated Unlearning]] framework designed to address critical challenges in [[Data Privacy]] and security within [[Federated Learning]] ecosystems. As the "right to be forgotten" becomes a regulatory necessity, models must be able to remove the influence of specific datasets without undergoing the computationally expensive process of retraining from scratch. Jellyfish ensures that once data is deleted, the global model no longer retains or discloses related information.

The Jellyfish scheme distinguishes itself from conventional [[Machine Learning]] unlearning frameworks through four core technical innovations:
...
2.  **Knowledge Disentanglement**: The system implements a mechanism to disentangle knowledge within the [[Neural Network]] architecture. By regularizing the output of the final [[Convolutional Neural Network]] layer, the scheme restricts the number of activated channels and encourages activation sparsity, effectively isolating and neutralizing the features related to the target data.
