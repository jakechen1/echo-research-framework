...
The research paper "Fatigue-Aware Learning to Defer via Constrained Optimisation" addresses a critical flaw in current [[learning-to-defer-l2d|Learning to Defer (L2D)]] frameworks: the assumption of static human performance. In practical [[human-ai-collaboration|Human-AI Collaboration]] scenarios, human experts are subject to performance degradation caused by cognitive workload and fatigue, a factor traditionally ignored by automated delegation models.
...
To manage this complexity, the paper formulates the L2D problem as a [[constrained-markov-decision-process-cmdp|Constrained Markov Decision Process (CMDP)]]. The decision-making state within this model is multi-dimensional, encompassing:
...
The optimization process is executed using **PPO-Lagrangian training**, a variation of [[proximal-policy-optimization-ppo|Proximal Policy Optimization (PPO)]], which allows the system to optimize for overall accuracy while strictly adhering to predefined human-AI cooperation budgets.
...
This work represents a significant step toward creating more resilient and biologically-aware [[artificial-intelligence-and-the-structure-of-mathematics|Artificial Intelligence]] systems capable of working alongside humans in high-stakes, long-duration environments.