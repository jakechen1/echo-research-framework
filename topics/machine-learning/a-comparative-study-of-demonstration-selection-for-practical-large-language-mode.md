...
This research investigates the optimization of [[Large Language Models]] (LLMs) for the specific task of [[Next POI Prediction]]. The study focuses on predicting a user's subsequent location based on historical check-in data, a critical component of modern [[Urban Computing]] and location-based services.

The core challenge addressed is the efficiency of [[In-Context Learning]] (ICL). In ICL, the performance of the model is heavily dependent on the "demonstrations"—the specific historical examples provided in the prompt to guide the output. While previous methodologies have attempted to use complex [[Machine Learning]] techniques, such as [[Embedding-based selection]], to choose the most relevant examples, this paper provides a much-needed comparative analysis of various selection strategies.
...
*   **Heuristic Approaches:** Utilizing [[Geographical Proximity]], [[Temporal Ordering]], and the identification of [[Sequential Patterns]].

The experimental results, derived from three real-world datasets, reveal that simpler [[Heuristic]] methods consistently outperform computationally expensive embedding-based methods. These heuristic strategies are superior in both predictive accuracy and [[Computational Cost]]. 

A significant finding of this study is that LLMs utilizing demonstrations selected through these efficient, simple heuristics can even outperform existing [[Fine-tuned models]] without the need for additional training or parameter updates. This suggests that for practical, large-scale deployments in [[Artificial Intelligence]], prioritizing efficient, rule-based demonstration selection is more effective than implementing high-overhead, computationally taxing architectures. This research offers a streamlined roadmap for implementing highly accurate prediction models in resource-constrained environments.