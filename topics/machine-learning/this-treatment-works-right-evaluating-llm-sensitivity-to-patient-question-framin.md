...
## Overview
This research investigates the reliability of [[Large Language Models]] (LLMs) when used for [[Medical Question Answering]] (QA). As patients increasingly rely on [[AI]] to interpret complex health data, the sensitivity of these models to how a question is phrased—known as "prompt sensitivity"—presents a significant risk to clinical safety.

## Methodology
The study employed a controlled [[Retrieval-Augmented Generation]] (RAG) environment. To ensure maximum control and eliminate errors stemming from poor retrieval, the researchers used expert-selected documents derived from [[Clinical Trials]] abstracts as the ground truth.
...
The results demonstrate that LLM responses in medical contexts can be systematically manipulated through phrasing alone, even when the underlying evidence remains constant. This lack of [[Robustness]] suggests that current RAG-based systems may not be suitable for high-stakes medical applications without new evaluation criteria designed to measure "phrasal invariance." Development of more stable, evidence-grounded architectures remains a primary challenge for the field.