...
Spectral Compact Training (SCT) is an innovative training methodology designed to overcome the "memory wall" inherent in training [[Large Language Models]] (LLMs) on consumer-grade hardware. As model parameters scale, the memory requirements for dense weight matrices often exceed the capacities of non-enterprise GPUs. SCT addresses this by replacing dense weight matrices with a permanent [[Singular Value Decomposition]] (SVD) structure.
