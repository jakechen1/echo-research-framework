...
The paper "Drifting Fields are not Conservative" explores the mathematical properties of [[drifting-models|Drifting Models]], a specialized class of [[generative-modeling-of-granular-flow-on-inclined-planes-using-conditional-flow-m|Generative Modeling]] designed to generate high-quality samples in a single forward pass. These models function by transporting initial samples toward a target data distribution using a vector-valued [[drift-field|Drift Field]].

## The Non-Conservative Problem
A fundamental question addressed in this research is whether the mechanism of following a drift field is mathematically equivalent to minimizing a scalar [[loss-function|Loss Function]]. In many physical and mathematical systems, vector fields are "conservative," meaning they can be expressed as the gradient of a scalar potential (often referred to as an energy function). 
...
The study notes that the [[gaussian-kernel|Gaussian Kernel]] serves as a unique exception to this rule; in the case of Gaussian normalization, the drift field remains exactly the gradient of a scalar function, preserving its conservative nature.
