...
The paper "Deep Kuratowski Embedding Neural Networks for Wasserstein Metric Learning" addresses a critical bottleneck in modern [[Machine Learning]] pipelines: the high computational cost associated with calculating pairwise [[Wasserstein distance]] ($W_2$). As data analysis tasks scale, computing the exact optimal transport metric becomes prohibitively expensive. Drawing inspiration from the classical [[Kuratowski embedding theorem]], the authors propose two neural architectures designed to learn an efficient approximation of the $W_2$ distance.
...
The resulting architectures can function as fast, differentiable surrogates, replacing the expensive $W_2$ oracle in complex [[Optimization]] and [[Data Analysis]] workflows.