...
Modern [[advanced-driver-assistance-systems|Advanced Driver Assistance Systems]] (ADAS) rely heavily on the deployment of [[deep-neural-networks|Deep Neural Networks]] (DNNs) for essential tasks like perception, object detection, and path planning. While these models are highly sophisticated, their physical implementation introduces significant vulnerabilities. During inference, the parameters of these DNNs reside in [[dram|DRAM]], making them susceptible to bit flips. These single-event upsets, often caused by cosmic radiation or low-voltage hardware operations, can corrupt computations and lead to catastrophic driving decisions, such as unintended steering or acceleration.
