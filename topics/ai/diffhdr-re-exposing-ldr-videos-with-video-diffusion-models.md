...
*   **Spatio-Temporal Priors**: By leveraging the learned weights of a [[diffhdr-re-exposing-ldr-videos-with-video-diffusion-models|Diffusion Model]], DiffHDR utilizes spatio-temporal generative priors. This ensures that the newly synthesized pixels are not only visually accurate but also temporally stable across video frames.
*   **Controllable Synthesis**: The framework allows for highly customized re-exposure through [[less-is-more-data-efficient-adaptation-for-controllable-text-to-video-generation|Text-to-Video]] guidance or the use of reference images, providing artists with granular control over the final aesthetic.
