...
ALPINE addresses these challenges through a closed-loop architecture designed for adaptive [[privacy budget]] allocation. The framework operates by performing multi-dimensional risk perception on terminal devices. This perception model evaluates four distinct risk vectors:
...
Extensive experiments on real-world datasets demonstrate that ALPINE outperforms representative baselines in maintaining data utility while preserving privacy. Specifically, the framework significantly reduces the effectiveness of several [[privacy attacks]], including:
* Membership inference attacks.
* Property inference attacks.
* Reconstruction attacks.

Furthermore, prototype deployment shows that ALPINE maintains robust performance for downstream tasks and introduces only modest runtime overhead, making it highly suitable for deployment on resource-constrained [[edge computing]] hardware.