...
## Overview
In the field of [[Physics-based character animation]], a primary goal is the synthesis of realistic and physically plausible motions. While modern [[Deep Reinforcement Learning]] (DRL) frameworks have successfully enabled characters to perform complex skills, they are fundamentally constrained by the laws of [[Classical Mechanics]]. Specifically, when modeling characters as underactuated, floating-base systems, [[momentum conservation]] strictly prohibits "exaggerated" or "stylized" motions—such as instantaneous dashes or sudden mid-air trajectory shifts—that are frequently required in [[Digital Humans]] and [[Computer Animation]].

## The Challenge of Instability
Previous attempts to force these "physically infeasible" motions by applying external wrenches (forces) have encountered significant hurdles. The introduction of external forces often results in velocity discontinuities, creating sparse, high-magnitude force spikes. These spikes lead to severe training instability, preventing the [[Neural Network]] policy from achieving convergence.
...
1.  **Analytic High-Frequency Component:** Derived through [[Inverse Dynamics]] to handle the rapid, structural requirements of the motion.
