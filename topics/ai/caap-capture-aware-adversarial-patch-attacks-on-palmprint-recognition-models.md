...
Testing conducted on the **Tongji**, **IITD**, and **AISEC** datasets demonstrates that CAAP achieves powerful untargeted and targeted attack success rates. Notably, the attack shows high [[cross-model-transferability|Cross-Model Transferability]], meaning the patch can successfully deceive different neural network architectures. 

The study also highlights that while [[nearest-neighbor-projection-removal-adversarial-training|Adversarial Training]] can provide some protection, it is insufficient to fully eliminate the threat of physically realizable attacks. These findings underscore a critical need for more robust [[cybersecurity|Cybersecurity]] defenses in biometric recognition systems.
