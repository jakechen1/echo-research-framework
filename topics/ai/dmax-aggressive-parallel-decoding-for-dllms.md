...
DMax represents a significant advancement in the efficiency of [[$s^3$-stratified-scaling-search-for-test-time-in-diffusion-language-models|Diffusion Language Models]] (dLLMs). While traditional masked dLLMs often struggle with [[error-accumulation|Error Accumulation]] during the decoding process, DMax introduces a new paradigm that enables highly aggressive [[dmax-aggressive-parallel-decoding-for-dllms|Parallel Decoding]] without sacrificing the quality of the generated text.

## Core Innovations

The fundamental shift in DMax is the movement away from the conventional binary mask-to-token transition. Instead, DMax reformulates the decoding process as a continuous, progressive [[self-refinement|Self-Refinement]] task, transitioning from mask embeddings to token embeddings.
...
Experimental results demonstrate that DMax significantly improves the Tokens Per Frame (TPF) metric—a measure of decoding speed—while maintaining high accuracy. When compared to the [[llada-20-mini|LLaDA-2.0-mini]] baseline, the improvements are notable:

* **GSM8K Benchmark:** TPF increased from 2.04 to 5.47.
* **MBPP Benchmark:** TPF increased from 2.71 to 5.86.

In terms of raw throughput, DMax achieves an average of 1,338 Tokens Per Second (TPS) on two [[nvidia-h200-gpus|NVIDIA H200 GPUs]] at a batch size of 1. This makes DMax a highly scalable solution for high-speed [[artificial-intelligence-and-the-structure-of-mathematics|Artificial Intelligence]] inference tasks requiring complex reasoning and coding capabilities.