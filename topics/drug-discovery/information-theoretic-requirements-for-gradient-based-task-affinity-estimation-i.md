...
The core contribution of this research is the discovery of a previously unstated assumption in [[gradient-based-analysis|gradient-based analysis]]. The authors demonstrate that for gradient-based metrics to reveal genuine task relationships, tasks must share training instances. 

When tasks are measured on the same inputs, the resulting gradient alignment accurately reflects shared [[mechanistic-structure|mechanistic structure]]. Conversely, when tasks are measured on disjoint inputs, any apparent signal in the gradients is not a reflection of task affinity, but rather a conflation of task relationships with [[generative-models-for-decision-making-under-distributional-shift|distributional shift]].
...
* **High Overlap (> 40%):** Gradient signals become reliable, successfully recovering known [[biological-pathways|biological pathways]] and organizational structures.
