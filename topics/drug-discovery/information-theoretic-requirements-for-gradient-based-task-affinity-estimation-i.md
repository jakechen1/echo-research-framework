...
When tasks are measured on the same inputs, the resulting gradient alignment accurately reflects shared [[mechanistic structure]]. Conversely, when tasks are measured on disjoint inputs, any apparent signal in the gradients is not a reflection of task affinity, but rather a conflation of task relationships with [[distributional shift]].
...
* **High Overlap (> 40%):** Gradient signals become reliable, successfully recovering known [[biological pathways]] and organizational structures.

## Impact on Standard Benchmarks
The paper highlights a critical failure in current [[machine-learning]] evaluation standards. Most widely used benchmarks fail to meet the necessary overlap threshold for valid gradient analysis:
* **MoleculeNet:** Operates with less than 5% overlap.
* **TDC (Therapeutics Data Commons):** Operates within a 8-14% overlap range.

By demonstrating that these datasets reside far below the 40% threshold, the authors provide the first principled explanation for the inconsistent MTL results reported in [[drug-discovery]] and related fields over the last seven years. This finding suggests that future research