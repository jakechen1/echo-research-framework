...
*   **Robustness to Visual Perturbations**: Through advanced [[Domain Randomization]] techniques integrated into the pre-training phase, the model showed unprecedented resilience to changes in lighting, camera angle, and visual occlusion.
...
4.  **Latency in Diffusion**: The use of [[Diffusion Policy]] for action generation, while highly accurate, introduces computational overhead that can limit the control frequency (Hz), potentially making the robot sluggish in high-speed reactive tasks.
