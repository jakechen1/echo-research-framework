...
if [[ "$1" == "--resolver" ]]; then
  ...
  if [ -n "$RED" ]; then
    ...
    # Auto-resolver attempts remediation before escalating
    $PY $WS/skills/auto-resolver/scripts/auto_resolver.py >> \
      /Users/jakeclaw/Library/Logs/auto_resolver.log 2>&1
  fi
fi
