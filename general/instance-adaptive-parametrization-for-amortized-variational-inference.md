...
dc.title: "Instance-Adaptive Parametrization for Amortized Variational Inference"
dc.creator: wiki-pipeline
dc.date: 2024-05-22
dc.type: Text
dc.format: text/markdown
dc.identifier: general/instance-adaptive-parametrization-for-amortized-variational-inference.md
dc.language: en
dc.rights: CC-BY-4.0
---
...
The research paper "Instance-Adaptive Parametrization for Amortized Variational Inference" introduces a novel framework known as the **Instance-Adaptive Variational Autoencoder (IA-VAE)**. This approach aims to solve a fundamental limitation in [[Deep Generative Modeling]] known as the **amortization gap**.

## The Amortization Gap
In standard [[posterior-calibrated-causal-circuits-in-variational-autoencoders-why-image-domai|Variational Autoencoders]] (VAEs), the model relies on **amortized variational inference**. In this setup, a single shared encoder is trained to map all input data to their respective posterior distributions. While this method is essential for the scalability of [[Latent Variable Models]], the reliance on a shared parameter set creates a bottleneck. The encoder is forced to learn a single, generalized mapping, which often fails to capture the complex, instance-specific nuances of the true posterior, leading to sub-optimal approximations.

## The IA-VAE Solution
The authors propose the IA-VAE framework, which introduces a mechanism for input-dependent adaptation. Instead of a static encoder, the architecture utilizes a [[Hypernetwork]] to generate modulations that are specific to each input instance. These modulations adapt the shared encoder's parameters on the fly. 
