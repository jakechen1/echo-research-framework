...
dc.title: "Correcting Source Mismatch in Flow Matching with Radial-Angular Transport"
dc.creator: wiki-pipeline
dc.date: 2024-05-23
dc.type: Text
dc.format: text/markdown
dc.identifier: general/correcting-source-mismatch-in-flow-matching-with-radial-angular-transport.md
dc.language: en
dc.rights: CC-BY-4.0
---
...
The paper "Correcting Source Mismatch in Flow Matching with Radial-Angular Transport" addresses a fundamental limitation in modern [[generative-modeling-of-granular-flow-on-inclined-planes-using-conditional-flow-m|Generative Modeling]]. Standard [[evoflows-evolutionary-edit-based-flow-matching-for-protein-engineering|Flow Matching]] architectures are typically constructed using a [[Gaussian Distribution]] as the base source and Euclidean probability paths. While this approach is highly efficient, it introduces a structural "source mismatch" when attempting to model data that is inherently heavy-tailed or anisotropic.
...
The authors provide a rigorous mathematical foundation for RAFM, establishing the exact density of the matched-radial source and proving a **radial-angular KL decomposition**. This decomposition is critical as it isolates the specific error penalty associated with Gaussian radial assumptions. Furthermore, the paper derives a stability result that links the error in [[evoflows-evolutionary-edit-based-flow-matching-for-protein-engineering|Flow Matching]] directly to the error in the final generated samples.

Empirically, RAFM substantially outperforms standard Gaussian-based Flow Matching on heavy-tailed and extreme-event data. Notably, it achieves these improvements while remaining competitive with other non-Gaussian alternatives and maintaining the lightweight, deterministic training pipeline essential for scalable [[artificial-intelligence-and-the-structure-of-mathematics|Artificial Intelligence]] applications.